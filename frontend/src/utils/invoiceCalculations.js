...
export const getLineAmountDetails = (item) => {
  ...
  let finalDiscountWei = 0n;
  if (discountType === "percentage") {
    finalDiscountWei = (lineTotalWei * discountWei) / HUNDRED_INVOICE_UNITS;
  } else {
    finalDiscountWei = discountWei;
  }

  let taxAmountWei = 0n;
  if (taxType === "percentage") {
    taxAmountWei = (lineTotalWei * taxRateWei) / HUNDRED_INVOICE_UNITS;
  } else {
    taxAmountWei = taxRateWei;
  }

  const amountWei = lineTotalWei - finalDiscountWei + taxAmountWei;
